...
def nyul_train_standard_scale(img_fns,
    ...
    for i, (img_fn, mask_fn) in enumerate(zip(img_fns, mask_fns)):
        ...
        img_data = sitk.GetArrayFromImage(sitk.ReadImage(img_fn))
        mask = sitk.ReadImage(mask_fn) if mask_fn is not None else None
        mask_data = img_data > img_data.mean() \
            if mask is None else sitk.GetArrayFromImage(mask)
