...
class RPN(nn.Module):
    ...
    def forward(self, x: torch.Tensor, img_size):
        ...
        anchor = _enumerate_shifted_anchor(
            self.anchor_base, self.feat_size, hh, ww)
        ...
        rpn_locs = rpn_locs.permute(0, 2, 3, 1).contiguous().view(
            n, -1, 4)  # 按照空间位置\anchor循序, 排列输出的locs
        rpn_scores = rpn_scores.permute(0, 2, 3, 1).contiguous()
        rpn_softmax_scores = F.softmax(rpn_scores.view(
            n, hh, ww, n_anchor, 2), dim=4)  # 只对每个anchor的正负例进行softmax
        rpn_fg_scores = rpn_softmax_scores[:,
                                           :, :, :, 1].contiguous()  # 只要物体的分数
        ...
        for i in range(n):
            roi, score = self.proposal_layer(
                rpn_locs[i], rpn_fg_scores[i], anchor, img_size)
...
def _enumerate_shifted_anchor(anchor_base: torch.Tensor, feat_stride, height, width):
    ...
    shift = torch.stack((shift_y.flatten(), shift_x.flatten(),
    ...
    anchor = anchor_base.reshape((1, A, 4)) + \
        shift.reshape((1, K, 4)).permute((1, 0, 2))
